...
#    test_loader = torch.utils.data.DataLoader(dataset2, **test_kwargs)
    ...
    ### transfer to tf onnx
    torch.save(best_model.state_dict(), 'tf/sf_lite.pth')

    # model = Net().to(device)
    dummy_input = Variable(torch.rand(1, 12, 7)).to(device)
    # torch.onnx.export(model, dummy_input, "tf/sf.onnx")
    torch.onnx.export(best_model, dummy_input, "tf/sf_lite.onnx",opset_version=10)

    new_model = onnx.load('tf/sf_lite.onnx')
    tf_rep = prepare(new_model) 
    tf_rep.export_graph('tf/sf_lite.pb')
    print('all ended...')
